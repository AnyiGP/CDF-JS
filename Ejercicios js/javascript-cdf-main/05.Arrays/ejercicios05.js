...
function cantPares(arr) {
  ...
  // Tu codigo: crear contador que lea cada valor y devolver los pares.
  let contador = 0;
...
function eliminarDuplicado(arr) {
  ...
  // Tu codigo:  index of y un slice
  let nuevoArray = [];
  for (let i = 0; i < arr.length; i++) {
    if (nuevoArray.indexOf(arr[i]) === -1) {
...
function dosEnUno(arr1, arr2) {
  ...
  //Tu codigo:  agragr un array a otro, se puede hacer con un for
...
function matrizTranspuesta(matriz) {
  ...
  // let matrizTranspuesta = new Array(columnas);
  // for (let i = 0; i < columnas; i++) {
  //   matrizTranspuesta[i] = new Array(filas);
  // }

  // // Llenar la matriz transpuesta intercambiando filas por columnas
